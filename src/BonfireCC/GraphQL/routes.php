...
], config('graphql.route_group_attributes', [])), function($router)
{
    ...
    // Controllers
    $controllers = config('graphql.controllers', \BonfireCC\GraphQL\GraphQLController::class . '@query');
        ...
        if(preg_match($schemaParameterPattern, $queryRoute))
        {
            ...
            foreach(config('graphql.schemas') as $name => $schema)
            {
                Route::match(
                    ...
                    BonfireCC\GraphQL\GraphQL::routeNameTransformer($name, $schemaParameterPattern, $queryRoute),
                    [
                        'uses'          => $queryController,
...
if (config('graphql.graphiql.display', true))
{
    ...
    {
        $graphiqlController =  config('graphql.graphiql.controller') ?? \BonfireCC\GraphQL\GraphQLController::class . '@graphiql';
        $schemaParameterPattern = '/\{\s*graphql\_schema\s*\?\s*\}/';
        foreach (config('graphql.schemas') as $name => $schema)
        {
            Route::match(
                ['get', 'post'],
                BonfireCC\GraphQL\GraphQL::routeNameTransformer($name, $schemaParameterPattern, '{graphql_schema?}'),
