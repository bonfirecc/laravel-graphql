...
```php
use BonfireCC\GraphQL\Support\Privacy;
use Auth;
...
The third argument passed to a query's resolve method is an instance of `BonfireCC\GraphQL\Support\SelectFields` which you can use to retrieve keys from the request. The following is an example of using this information to eager load related Eloquent models.
...
than 'total'), just copy the entry for the field you want to replace (they're in BonfireCC/GraphQL/Support/PaginationType.php) 
...
use BonfireCC\GraphQL\Support\UnionType;
...
use GraphQL;
use BonfireCC\GraphQL\Support\InterfaceType;
use GraphQL\Type\Definition\Type;
